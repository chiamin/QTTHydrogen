...
def one_over_r_1D (inds, fac=1., rescale=1., shift=0., cutoff=1e-12):
    N = len(inds)
...
def one_over_r_2D (inds, fac=1., rescale=1., shift=0., cutoff=1e-12):
    N = len(inds)
    inds_x = inds[:N//2]
    inds_y = inds[N//2:]
    ...
    r = np.sqrt(x*x+y*y)
    #print(x,y,r,'|',rescale,shift)
    if np.abs(r) > cutoff:
        return -fac/np.abs(r)
    else:
        return -fac/cutoff

def one_over_r_3D (inds, fac=1., rescale=1., shift=0., cutoff=1e-12):
    N = len(inds)
    Ni = N//3
    inds_x = inds[:Ni]
    inds_y = inds[Ni:2*Ni]
...
if __name__ == '__main__':
    ...
    if '--2D_one_over_r' in sys.argv:
        ...
        mps = run_tci (target_func, nsite, rescale, shift, cutoff, factor)
    elif '--1D_one_over_r' in sys.argv:
        def target_func (inds):
