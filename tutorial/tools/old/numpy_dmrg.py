...
class LR_envir_tensors_mpo:
    ...
    def update_LR (self, mps1, mps2, mpo, centerL, centerR=None):
        ...
        for p in range(self.centerL, centerL):
            ...
            #  |  |---------O--- -4
            #  ----    1
            tmp = ncon((self.LR[p-1],mps1[p]),((-1,-2,1),(1,-3,-4)))
            #  ----
            #  |  |--- -1
            #  |  |        -2
            #  |  |         |
...
class eff_Hamilt_0site (sp.sparse.linalg.LinearOperator):
    def __init__ (self, L, R, dtype=float):
        self.L = L
        self.R = R
        self.dtype = np.dtype(dtype)
...
def dmrg (numCenter, psi, H, L, R, maxdims, cutoff, krylovDim=20, verbose=False):
    ...
    L0 = L.reshape((1,len(L),1))
    R0 = R.reshape((1,len(R),1))
...
def tdvp (numCenter, psi, H, L, R, dt, maxdims, cutoff, krylovDim=20, verbose=False):
    # Check the MPS and the MPO
    assert (len(psi) == len(H))
    npmps.check_MPO_links (H, L, R)
    npmps.check_MPS_links (psi)
    ...
    for k in range(len(maxdims)):                                                            # For each sweep
        ...
        for lr in [0,1]:
            for p in ranges[lr]:
                ...
                dims = phi.shape
                phi = phi.reshape(-1)
                phi = lanczos.lanczos_expm_multiply (effH, phi, -0.5*dt, k=krylovDim)
                phi = phi.reshape(dims)
                # Normalize phi
                phi = phi / np.linalg.norm(phi)
                ...
                if (lr == 0 and p+numCenter-1 != Nsites-1) or (lr == 1 and p != 0):
                    ...
                    if numCenter == 2:
                        ...
                        # wavefunction for back propagation
                        if toRight:
                            psi[p], R = npmps.qr_decomp (psi[p], rowrank=2, toRight=toRight)
                        else:
                            psi[p], R = npmps.qr_decomp (psi[p], rowrank=1, toRight=toRight)
                        phi = R
                        LR.delete(p)
                        # Update the environment tensors
