...
def apply_op_MPS (mps, op, i):
    check_MPS_links (mps)
    assert op.ndim == 2
    assert op.shape[0] == op.shape[1]

    mps = copy.copy(mps)
    mps[i] = ncon([mps[i],op], ((-1,1,-3),(1,-2)))
    return mps
...
def MPO_to_matrix (mpo, L, R):
    mpo = copy.copy(mpo)
    re = ncon([L,mpo[0]], ((1,),(1,-1,-2,-3)))
    re = re.reshape((1,*re.shape))
    mpo[-1] = ncon((mpo[-1],R), ((-1,-2,-3,1),(1,)))
    mpo[-1] = mpo[-1].reshape((*mpo[-1].shape,1))
    N = len(mpo)
    for i in range(1,N):
        hi = mpo[i]
        d1 = [-1] + [-2-j for j in range(i)] + [-i-3-j for j in range(i)] + [1]
        d2 = [1, -i-2, -2*i-3, -2*i-4]
        print(d1,d2)
        re = ncon([re,hi], (d1, d2))
    re = re.reshape((2**N,2**N))
    return re
